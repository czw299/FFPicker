...
class NotificationBarSetter {
    companion object{
        fun setNotificationBar(activity: Activity, isFullscreen: Boolean, isNotificationBarLightMode: Boolean, isNavegationLightMode: Boolean) {
            val integerArrayList = ArrayList<Int>()
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                integerArrayList.add(View.SYSTEM_UI_FLAG_LAYOUT_STABLE)
                if(isNotificationBarLightMode){
                ...
                }else if (!isNavegationLightMode && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O_MR1){
                    activity.window.navigationBarColor = Color.parseColor("#000000")
                }
                if (isFullscreen) {
        ...
        fun setNotificationBarWhiteFont(activity: Activity, isFullscreen: Boolean){
            val integerArrayList = ArrayList<Int>()
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                integerArrayList.add(View.SYSTEM_UI_FLAG_LAYOUT_STABLE)
                if (isFullscreen) {
